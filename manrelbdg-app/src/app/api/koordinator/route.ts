...
// GET /api/koordinator - Get all koordinator with pagination
export async function GET(request: NextRequest) {
  try {
    ...
    const [koordinator, total] = await Promise.all([
      ...
      db.koordinator.count({ where })
    ])
    
    return paginatedResponse(koordinator, { ...pagination, total })
    
  } catch (error) {
    console.error('Get koordinator error:', error)
    return serverErrorResponse('Failed to fetch koordinator data')
  }
}

// POST /api/koordinator - Create new koordinator
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate input
    const validation = koordinatorSchema.safeParse(body)
    ...
    const existingKoordinator = await db.koordinator.findUnique({
      where: { nik: data.nik }
    })
    ...
    const count = await db.koordinator.count()
    const kode = `KOR${String(count + 1).padStart(4, '0')}`
    
    // Create koordinator
    const koordinator = await db.koordinator.create({
      data: {
        ...data,
        kode,
    ...
    return successResponse(koordinator, 'Koordinator berhasil ditambahkan', 201)
    
  } catch (error) {
    console.error('Create koordinator error:', error)
    return serverErrorResponse('Failed to create koordinator')
  }
}
