...
export const useTerminology = () => {
  return CLIENT_CONFIG.content.terminology
}
...
export const isFeatureEnabled = (featurePath: string): boolean => {
  ...
  let current: Record<string, unknown> = CLIENT_CONFIG.features
...
export const getText = (path: string, fallback?: string): string => {
  ...
  let current: Record<string, unknown> = CLIENT_CONFIG.content
  
  for (const pathSegment of pathArray) {
    if (current[pathSegment] === undefined) {
      return fallback || path
    }
    current = current[pathSegment] as Record<string, unknown>
  }
